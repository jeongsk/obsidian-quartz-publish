...
import { ICON_QUARTZ_PUBLISH } from "../constants/icons";
...
export class QuartzPublishSettingTab extends PluginSettingTab {
	...
	private createGitHubSection(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.addText((text) => {
				...
				text.setPlaceholder("Auto-detected on connection test")
	...
	private openRemoteFileManagerModal(): void {
		const { githubToken, repoUrl, defaultBranch, contentPath } =
	...
	private createFrontmatterSection(containerEl: HTMLElement): void {
		new Setting(containerEl)
			.setName(t("settings.frontmatter.title"))
			.setHeading();
	...
	private async testConnection(): Promise<void> {
		...
		try {
			...
			if (result.success && result.repository) {
				...
				if (
					detectedBranch &&
					detectedBranch !== this.plugin.settings.defaultBranch
				) {
	...
	private connectionStatusTimeout: ReturnType<typeof setTimeout> | null =
		null;
	...
	private createQuartzSettingsSection(containerEl: HTMLElement): void {
		new Setting(containerEl)
			.setName(t("settings.quartz.title"))
			.setHeading();
	...
	private async loadQuartzSettings(): Promise<void> {
		if (!this.quartzSettingsContainerEl || this.isQuartzSettingsLoading)
			return;
		...
		try {
			...
			if (!connectionResult.success) {
				...
				let errorMsg =
					connectionResult.error?.message ?? "Unknown error";
	...
	private renderAdvancedConfigSection(config: ParsedQuartzConfig): void {
		if (!this.quartzSettingsContainerEl || !this.quartzConfigService)
			return;
		...
		const siteConfig =
			this.quartzConfigService.toQuartzSiteConfig(extendedConfig);
		...
		this.advancedConfigContainerEl =
			this.quartzSettingsContainerEl.createDiv({
				cls: "quartz-publish-advanced-config",
			});

		// Unsaved Warning Banner (T035, T042)
		this.unsavedWarning = new UnsavedWarning(
			this.advancedConfigContainerEl,
			{
				onDiscard: () => {
		...
		this.siteInfoSection = new SiteInfoSection(
			this.advancedConfigContainerEl,
			{
				config: {
		...
		this.analyticsSection = new AnalyticsSection(
			this.advancedConfigContainerEl,
			{
		...
		this.typographySection = new TypographySection(
			this.advancedConfigContainerEl,
			{
				app: this.app,
				...
				onChange: (field, value) => {
					...
					const currentConfig =
						this.pendingChangesManager?.getCurrentConfig();
					if (currentConfig) {
						const newTypography = {
		...
		this.publishingSection = new PublishingSection(
			this.advancedConfigContainerEl,
			{
				config: {
	...
	private async handleApplyChanges(): Promise<void> {
		...
		try {
			...
			const commitMessage =
				this.pendingChangesManager.generateCommitMessage();
	...
	private async performUpgrade(): Promise<void> {
		...
		const onProgress = (progress: QuartzUpgradeProgress) => {
			...
			statusEl.textContent =
				statusTexts[progress.status] || progress.status;
