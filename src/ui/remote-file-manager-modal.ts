...
export class RemoteFileManagerModal extends Modal {
		...
		try {
			...
			if (!forceRefresh && this.cachedFiles) {
				this.updateState({
					...
					duplicateGroups: this.remoteFileService.detectDuplicates(
						this.cachedFiles
					),
					isLoading: false,
				});
	...
	private renderFileList(container: HTMLElement): void {
		...
		const duplicateFileNames = new Set(
			this.state.duplicateGroups.map((g) => g.fileName)
		);
	...
	private toggleSelectAll(): void {
		...
		const allSelected =
			this.state.selectedFiles.size === this.state.filteredFiles.length;
	...
	private handleSearch(query: string): void {
		const filteredFiles = this.remoteFileService.searchFiles(
			this.state.files,
			query
		);
		this.updateState({
	...
	private async handleDelete(): Promise<void> {
		...
		try {
			...
			if (result.allSucceeded) {
				new Notice(
					t('modal.remoteFiles.deleteSuccess', { count: result.succeeded.length })
				);
			} else {
				new Notice(
