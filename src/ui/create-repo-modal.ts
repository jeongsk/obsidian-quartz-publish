...
export class CreateRepoModal extends Modal {
	...
	private renderForm() {
		...
		const isDisabled =
			this.state === "validating" || this.state === "creating";

		new Setting(contentEl)
			.setName(t("modal.createRepo.name"))
			.setDesc(t("modal.createRepo.nameDesc"))
			.addText((text) => {
				text.setPlaceholder(DEFAULT_REPO_NAME)
		...
		const createBtn = buttonContainer.createEl("button", {
			text:
				this.state === "creating"
					? t("modal.createRepo.creating")
					: t("modal.createRepo.create"),
	...
	private async handleCreate() {
		...
		try {
			...
			if (result.success) {
				...
				new Notice(
					`Repository "${result.repository.fullName}" created successfully!`,
				);
			} else {
				this.state = "error";
