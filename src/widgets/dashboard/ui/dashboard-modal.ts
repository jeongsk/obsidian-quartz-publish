...
import { TAB_LABELS } from "../../../app/types";
import type { NetworkService } from "../../../shared/lib/network";
import { FileValidatorService } from "../../../shared/lib/file-validator";
import { LargeFileWarningModal } from "../../../features/publish-note/ui/large-file-warning-modal";
import { MAX_FILE_SIZE } from "../../../app/types";
...
export class DashboardModal extends Modal {
  ...
  private toggleSelectAll(): void {
    ...
    const allSelected = currentNotes.every((note) => this.state.selectedPaths.has(note.file.path));

    if (allSelected) {
      // 전체 해제
      currentNotes.forEach((note) => this.state.selectedPaths.delete(note.file.path));
    } else {
      // 전체 선택
      currentNotes.forEach((note) => this.state.selectedPaths.add(note.file.path));
    }
    this.render();
  }
  ...
  private renderProgress(): void {
    const progressEl = this.contentEl.querySelector(".quartz-publish-loading-progress");
    if (!progressEl || !this.progressInfo) return;

    const percentage =
      this.progressInfo.total > 0
        ? Math.round((this.progressInfo.current / this.progressInfo.total) * 100)
        : 0;

    const progressBar = progressEl.querySelector(".quartz-publish-progress-bar") as HTMLElement;
    if (progressBar) {
      progressBar.style.width = `${percentage}%`;
    }

    const progressText = progressEl.querySelector(".quartz-publish-loading-text");
    if (progressText) {
  ...
  private handleTabKeydown(e: KeyboardEvent, currentIndex: number): void {
    ...
    switch (e.key) {
      case "ArrowLeft":
        newIndex = currentIndex === 0 ? TAB_KEYS.length - 1 : currentIndex - 1;
        break;
      case "ArrowRight":
        newIndex = currentIndex === TAB_KEYS.length - 1 ? 0 : currentIndex + 1;
        break;
      case "Home":
        newIndex = 0;
        break;
    ...
    // 새 탭에 포커스 이동
    const newTabEl = this.contentEl.querySelector(`#tab-${newTab}`) as HTMLElement;
    newTabEl?.focus();
  }
  ...
  private renderNoteList(): void {
    ...
    if (notes.length > 0) {
      ...
      const allSelected =
        notes.length > 0 && notes.every((note) => this.state.selectedPaths.has(note.file.path));
  ...
  private renderNoteItem(container: HTMLElement, note: NoteStatus): void {
    ...
    checkboxEl.addEventListener("change", () => this.toggleSelection(note.file.path));
  ...
  private renderActions(): void {
    ...
    } else if (this.state.activeTab === "new" || this.state.activeTab === "modified") {
