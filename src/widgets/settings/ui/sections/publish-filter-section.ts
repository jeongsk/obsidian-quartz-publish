...
import type { PublishFilterSettings } from "../../../../app/types";
import { t } from "../../../../shared/lib/i18n";
import { NoteSuggestModal } from "../../../../shared/ui/note-suggest-modal";
import { FolderSuggestModal } from "../../../../shared/ui/folder-suggest-modal";
...
export class PublishFilterSection {
  ...
  private renderRootFolder(): void {
    ...
    this.validateFolder(this.options.config.rootFolder, this.rootFolderWarningEl);
  }
  ...
  private renderHomePage(): void {
    ...
    this.validateHomePage(this.options.config.homePagePath, this.homePageWarningEl);
  }
  ...
  private renderIncludeFolders(): void {
    ...
    this.validateFolders(this.options.config.includeFolders, this.includeFoldersWarningEl);
  }
  ...
  private renderExcludeFolders(): void {
    ...
    this.validateFolders(this.options.config.excludeFolders, this.excludeFoldersWarningEl);
  }
  ...
  private validateFolders(folders: string[], warningEl: HTMLElement | null): void {
    if (!warningEl) return;
  ...
  private validateFolder(folder: string, warningEl: HTMLElement | null): void {
    if (!warningEl) return;
  ...
  private validateHomePage(path: string, warningEl: HTMLElement | null): void {
    if (!warningEl) return;
