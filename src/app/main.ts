...
import { StatusService } from "../features/sync-content/model/status";
import { NetworkService } from "../shared/lib/network";
import { ContentTransformer } from "../entities/note/lib/transformer";
import { PublishRecordStorage } from "../entities/publish-record/model/storage";
...
export default class QuartzPublishPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.statusService = new StatusService({
      ...
      getFilterSettings: () =>
        this.settings.publishFilterSettings ?? DEFAULT_PUBLISH_FILTER_SETTINGS,
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof TFile && file.extension === "md") {
          menu.addItem((item) => {
            item
              .setTitle(t("menu.publishToQuartz"))
              .setIcon("upload")
