...
export class RepositoryCreatorService {
	...
	private createError(type: RepositoryCreationErrorType, details?: string): RepositoryCreationError {
		return {
			type,
	...
	validateRepositoryName(name: string): { valid: boolean; error?: string } {
		...
		if (!REPO_NAME_REGEX.test(trimmedName)) {
			return { 
				valid: false, 
				error: '영문, 숫자, 점(.), 하이픈(-), 언더스코어(_)만 사용 가능합니다. 영문 또는 숫자로 시작하고 끝나야 합니다.' 
			};
		}
