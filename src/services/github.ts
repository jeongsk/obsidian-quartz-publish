...
export class GitHubService {
  ...
  private encodePath(path: string): string {
    return path.normalize("NFC").split("/").map(encodeURIComponent).join("/");
  }
  ...
  async getFile(path: string): Promise<GitHubFileContent | null> {
    ...
    try {
      ...
      if (error instanceof GitHubError && error.statusCode === 404) {
        const nfdPath = path.normalize("NFD").split("/").map(encodeURIComponent).join("/");
        ...
        if (nfcPath !== nfdPath) {
          try {
            ...
            if (retryError instanceof GitHubError && retryError.statusCode === 404) {
              console.log(
                "[GitHubService.getFile] Both NFC and NFD failed, trying filename search"
              );
  ...
  private processFileResponse(response: GitHubContentsResponse): GitHubFileContent | null {
   ...
   */
  async getDirectoryContents(path: string, recursive: boolean = true): Promise<PublishedFile[]> {
    // 경로를 NFC 정규화하고 URL 인코딩
    const encodedPath = this.encodePath(path);
