...
import type { PublishRecord, NoteStatus, StatusOverview, PublishFilterSettings } from "../types";
...
export class StatusService {
  ...
  async calculateStatusOverview(
    ...
    for (let i = 0; i < total; i += StatusService.CHUNK_SIZE) {
      ...
      // 병렬로 상태 계산
      const statuses = await Promise.all(chunk.map((file) => this.calculateFileStatus(file)));
  ...
  async calculateFileStatus(file: TFile): Promise<NoteStatus> {
    ...
    if (currentHash !== record.contentHash) {
      console.log(`[StatusService] Local hash mismatch for ${file.path}`);
      console.log(`  Local: ${currentHash}`);
      console.log(`  Record: ${record.contentHash}`);
      return {
        file,
        status: "modified",
  ...
  findDeletedNotes(): NoteStatus[] {
    ...
    for (const [path, record] of Object.entries(records)) {
      ...
      if (isCacheValid && remoteCache) {
        const remotePathNFC = record.remotePath.normalize("NFC");
        const existsInRemote = remoteCache.files.some(
          (f) => f.path.normalize("NFC") === remotePathNFC
        );

        // 원격에도 없으면 이미 삭제된 것으로 간주, 제외
        if (!existsInRemote) {
          console.log(
            `[StatusService] Skipping ${record.remotePath} - already deleted from remote`
          );
          continue;
        }
      }
  ...
  async syncWithRemote(
    ...
    try {
      ...
      // 2. 원격 파일 가져오기
      const result = await this.remoteSyncService.fetchRemoteFiles(onProgress);
      ...
      onProgress?.(
        t("dashboard.remoteSync.progress", {
  ...
  private async isRemoteChanged(record: PublishRecord): Promise<boolean> {
    ...
    const recordRemotePath = record.remotePath.normalize("NFC");

    // 원격 파일 찾기
    const remoteFile = cache.files.find((f) => {
      // remotePath로 비교 (NFC 정규화)
      return f.path.normalize("NFC") === recordRemotePath;
    });

    // 원격에 파일이 없으면 변경된 것으로 간주
    if (!remoteFile) {
      console.log(`[StatusService] Remote file missing for ${record.localPath}`);
      return true;
    }

    // SHA 비교
    if (remoteFile.sha !== record.remoteSha) {
      console.log(`[StatusService] Remote sha mismatch for ${record.localPath}`);
      console.log(`  Record remote path: ${record.remotePath}`);
      console.log(`  Remote file path: ${remoteFile.path}`);
      console.log(`  Record SHA: ${record.remoteSha}`);
      console.log(`  Remote SHA: ${remoteFile.sha}`);
      return true;
    }
