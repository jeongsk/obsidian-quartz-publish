...
import { DEFAULT_REMOTE_FILE_MANAGER_CONFIG } from '../types';
import { t } from '../i18n';
...
export class RemoteFileService {
	...
	async getPublishedFiles(): Promise<PublishedFile[]> {
		const files = await this.gitHubService.getDirectoryContents(
		...
		const filteredFiles = files.filter((file) =>
			this.config.fileExtensions.some((ext) =>
	...
	async deleteFiles(
		...
		if (files.length > this.config.maxBatchDelete) {
			throw new Error(
				t('error.remoteFiles.maxFiles', { max: this.config.maxBatchDelete })
			);
		}
