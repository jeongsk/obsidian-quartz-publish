...
import type { RemoteFileInfo, RemoteSyncResult } from "../types";
...
export class RemoteSyncService {
   ...
   * @returns 원격 동기화 결과
   */
  async fetchRemoteFiles(onProgress?: RemoteSyncProgressCallback): Promise<RemoteSyncResult> {
    try {
  ...
  private filterMarkdownFiles(
    ...
    const normalizedContentPath = this.contentPath.normalize("NFC");
    ...
    for (const item of tree) {
      ...
      if (
        !normalizedPath.startsWith(normalizedContentPath + "/") &&
        normalizedPath !== normalizedContentPath
      )
        continue;

      // .md 파일만 포함
      if (!normalizedPath.endsWith(".md")) continue;

      files.push({
        path: normalizedPath, // 정규화된 경로 저장
        sha: item.sha,
        size: item.size ?? 0,
        type: "blob",
      });
