...
import { DashboardModal, ConfirmModal } from "../../../src/widgets/dashboard/ui/dashboard-modal";
...
describe("DashboardModal - 전체 동기화", () => {
  ...
  it("삭제가 포함된 경우 확인 모달이 표시된다", async () => {
    ...
    // ConfirmModal 스파이
    const waitForConfirmationSpy = vi
      .spyOn(ConfirmModal.prototype, "waitForConfirmation")
  ...
  it("부분 실패 시 결과를 표시한다", async () => {
    ...
    onDelete.mockResolvedValue([
      { success: false, file: { path: "notes/deleted1.md" }, error: "Network error" },
    ]);
...
describe("ConfirmModal", () => {
  ...
  it("확인 버튼 클릭 시 true를 반환한다", async () => {
    ...
    const confirmButton = Array.from(buttons).find(
      (btn) => btn.classList.contains("mod-cta") || btn.classList.contains("mod-warning")
    );
